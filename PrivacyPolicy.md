...
- 相機與使用者選取的圖片：用於在裝置上辨識 QR Code／Aztec 條碼。你可透過 Android Photo Picker 選取既有圖片；應用程式不會將相機影像或選取的原始圖片保存為掃描歷史。
...
App 可能使用相機、網路、Wi-Fi 狀態／設定與震動權限。相機只用於掃描；從 Photo Picker 選取圖片不需要 App 取得整個相簿的讀取權限；Wi-Fi 內容會交由 Android 系統的加入網路流程處理，不由 App 靜默連線。
